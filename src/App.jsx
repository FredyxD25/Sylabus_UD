...
export default function App() {
  ...
  const [panel, setPanel]               = useState(null) // 'actualizar' | 'convertir' | null
  ...
  const carpetasUnicas = useMemo(() =>
    ['todas', ...new Set(archivos.map(a => a.carpeta))], [archivos])
  ...
  const actualizarCampos = async () => {
    ...
    setCargando(true)
    try {
      const res = await api.actualizarCampos({ carpeta, nuevaFecha })
  ...
  return (
    <div style={s.root}>
      ...
      <aside style={s.sidebar}>
        ...
        <button style={s.btnAccion} onClick={() => setPanel('actualizar')} disabled={!carpeta}>
          🔄 Actualizar versión / fecha
        </button>
        <button style={s.btnAccion} onClick={() => setPanel('convertir')} disabled={!carpeta}>
          📄 Convertir Excel → PDF
        </button>
        ...
        {['todos','pdf','xlsx'].map(t => (
          <button key={t} style={filtroTipo === t ? s.filtroActivo : s.filtro}
        ...
        <div style={s.logBox}>
          {log.slice().reverse().map((l, i) => (
            <div key={i} style={{...s.logLine, color: l.tipo === 'error' ? '#e05b5b' : l.tipo === 'ok' ? '#3ecf8e' : '#5a6080'}}>
        ...
        <div style={s.toolbar}>
          ...
          <div style={s.ordenGroup}>
            ...
            {['nombre','fecha','tamaño'].map(o => (
              <button key={o} style={orden === o ? s.ordenActivo : s.ordenBtn}
                onClick={() => setOrden(o)}>{o}</button>
            ))}
          </div>
        ...
        {cargando ? (
          ...
          <div style={s.grid}>
            {archivosFiltrados.map(a => (
              <div key={a.ruta}
                style={{...s.card, ...(seleccionados.has(a.ruta) ? s.cardSel : {})}}
                onClick={() => toggleSeleccion(a.ruta)}
                ...
                <div style={s.cardMeta}>
                  <span style={{...s.tipoBadge, background: a.tipo === 'pdf' ? '#e05b5b22' : '#4f6ef722',
        ...
        <div style={s.overlay} onClick={() => setPanel(null)}>
          <div style={s.modal} onClick={e => e.stopPropagation()}>
            {panel === 'actualizar' && <>
              ...
              <p style={s.muted}>Actualiza "Versión: 01 → 02" y la fecha de aprobación en todos los Excel.</p>
              ...
              <div style={s.modalBtns}>
                ...
                <button style={s.btnPrimary} onClick={actualizarCampos} disabled={!nuevaFecha}>
                  Ejecutar
                </button>
              </div>
            </>}
...
const s = {
  ...
  btnPrimary:  { background:'#4f6ef7', color:'#fff', border:'none', borderRadius:8, padding:'9px 14px', fontWeight:700, fontSize:13, cursor:'pointer', transition:'opacity .2s' },
  ...
  btnAccion:   { background:'#12151f', color:'#e8eaf0', border:'1px solid #1e2333', borderRadius:8, padding:'8px 12px', fontSize:12, fontWeight:600, textAlign:'left', cursor:'pointer', transition:'border-color .2s' },
  ...
  card:        { background:'#0f1117', border:'1px solid #1e2333', borderRadius:12, padding:14, cursor:'pointer', position:'relative', transition:'border-color .2s', display:'flex', flexDirection:'column', gap:6 },
