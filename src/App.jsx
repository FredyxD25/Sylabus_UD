...
export default function App() {
  ...
  const seleccionarTodos = () => {
    const todosSeleccionados =
      seleccionados.size === archivosFiltrados.length &&
      archivosFiltrados.length > 0;
    if (todosSeleccionados) {
      setSeleccionados(new Set());
    } else {
      setSeleccionados(new Set(archivosFiltrados.map((a) => a.ruta)));
    }
  };
  ...
  const unirPDFs = async () => {
    const pdfsLista = [...seleccionados].filter((r) =>
      r.toLowerCase().endsWith(".pdf"),
    );
    if (pdfsLista.length < 2) {
  ...
  const actualizarCodigo = async () => {
    if (!carpeta) return;
    iniciarCarga("Actualizando código en archivos Excel...");
    setPanel(null);
    try {
      await api.actualizarCodigo({ carpeta });
      agregarLog("Código AA-FR-003 → CC-FR-003 actualizado", "ok");
      cargarArchivos(carpeta);
    } catch (e) {
      agregarLog(`Error: ${e}`, "error");
      setCargando(false);
    ...
    <div style={s.root}>
      <aside style={s.sidebar}>
        ...
        <button
          style={{
            ...s.btnAccion,
            ...(accionesDesactivadas ? s.btnDesactivado : {}),
          }}
        ...
        <button
          ...
          onClick={() => !accionesDesactivadas && setPanel("codigo")}
          disabled={accionesDesactivadas}
        >
          🔁 Actualizar código
        </button>
      ...
      <main style={s.main}>
        <div style={s.toolbar}>
          ...
          {carpeta && archivosFiltrados.length > 0 && (
            <button
              style={{
                ...s.btnSelTodos,
                ...(todosSeleccionados ? s.btnSelTodosActivo : {}),
              }}
          ...
          <div style={s.grid}>
            {archivosFiltrados.map((a) => (
              <div
                ...
                style={{
                  ...s.card,
                  borderColor: seleccionados.has(a.ruta)
                    ? "#4f6ef7"
                    : "#1e2333",
                  background: seleccionados.has(a.ruta)
                    ? "#4f6ef710"
                    : "#0f1117",
                }}
      ...
      {panel && !cargando && (
        <div style={s.overlay} onClick={() => setPanel(null)}>
          <div style={s.modal} onClick={(e) => e.stopPropagation()}>
            ...
            {panel === "codigo" && (
              <>
                <h2 style={s.modalTitle}>🔁 Actualizar Código</h2>
                <p style={s.muted}>
                  Reemplaza{" "}
                  <b style={{ color: "#e8eaf0" }}>"Código: AA-FR-003"</b> por{" "}
                  <b style={{ color: "#4f6ef7" }}>"Código: CC-FR-003"</b> en
                ...
                <div style={s.modalBtns}>
                  ...
                  <button style={s.btnPrimary} onClick={actualizarCodigo}>
                    Ejecutar
                  </button>
                </div>
              </>
